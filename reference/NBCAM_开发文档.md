...
| `mainwindow.*` | 主窗口、菜单、配置对话框、加工控制、日志、路径保存、验证示例 |
...
- 打开手动操作面板。当前是占位界面，后续可接手动 jog、回零、激光测试等功能。
...
轴模式仅影响默认板卡的 A/B 编码。RTC-6 当前配置结构只包含 X/Y/Z 轴范围和 RTC 单位系数。
...
段级反馈开启时，每段发送后等待发送线程排空；关闭时连续写入本地缓冲，结束后统一等待排空。
...
- 路径规划和验证示例会在 `log` 或 `log/test` 下生成连接帧日志。
- `frame_log_utils` 可导出帧内容和首帧发送验证信息，便于与 C# 参考实现、板卡抓包对照。
