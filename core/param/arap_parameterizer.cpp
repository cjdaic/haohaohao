...
// 使用ARAP进行参数化
bool parameterizeWithARAP(Surface_mesh& mesh, UV_pmap uv_map, ParameterizationResult& result,
    ...
    try {
        ...
        // 使用ARAP参数化（As-Rigid-As-Possible，类似BFF的效果）
        CGAL::Surface_mesh_parameterization::ARAP_parameterizer_3<Surface_mesh> parameterizer;
        ...
        if (err != CGAL::Surface_mesh_parameterization::OK) {
            spdlog::error("ARAP参数化失败，错误代码: {}", static_cast<int>(err));
            return false;
        }
        ...
        for (auto v : vertices(mesh)) {
            ...
            if (it == cgal_to_original_index.end()) {
                spdlog::warn("ARAP参数化: 无法找到CGAL顶点的原始索引，跳过该顶点");
                continue;
            }
            
            size_t idx = it->second;
            if (idx >= result.uv_coords.size()) {
                spdlog::warn("ARAP参数化: 顶点索引 {} 超出范围 (总数: {})", idx, result.uv_coords.size());
                continue;
            }
            ...
            if (std::isfinite(u) && std::isfinite(v_val)) {
                ...
                valid_uv[idx] = false;
                spdlog::warn("ARAP参数化: 顶点 {} 产生无效UV坐标 ({}, {})", idx, u, v_val);
            }
        }
        ...
        for (size_t i = 0; i < valid_uv.size(); ++i) {
            if (!valid_uv[i]) {
                invalid_count++;
                spdlog::warn("ARAP参数化: 顶点 {} 的UV坐标无效（NaN或Inf），这可能是参数化过程中的错误", i);
            }
        }
        
        if (invalid_count > 0) {
            spdlog::error("ARAP参数化: 发现 {} 个无效UV坐标，这通常表示参数化过程存在问题", invalid_count);
            ...
            if (u_max > u_min && v_max > v_min) {
                for (size_t i = 0; i < valid_uv.size(); ++i) {
                    if (!valid_uv[i]) {
                        ...
                        spdlog::warn("ARAP参数化: 临时修复顶点 {} 的UV坐标为范围中心 ({:.6f}, {:.6f})", 
        ...
        result.island_count = 1;  // ARAP通常产生单个UV岛
        result.success = true;
        
        return true;
    } catch (const std::exception& e) {
        spdlog::error("ARAP参数化异常: {}", e.what());
        return false;
    }
}

ParameterizationResult ARAPParameterizer::parameterize(const TriangleMesh& mesh) {
    ...
    try {
        ...
        // 使用ARAP进行参数化
        if (!parameterizeWithARAP(cgal_mesh, uv_map, result, cgal_to_original_index)) {
            // 如果ARAP失败，尝试使用Discrete Authalic
            spdlog::warn("ARAP参数化失败，尝试使用Discrete Authalic");
            ...
            if (bhd != boost::graph_traits<Surface_mesh>::null_halfedge()) {
                CGAL::Surface_mesh_parameterization::Discrete_authalic_parameterizer_3<Surface_mesh> parameterizer;
                CGAL::Surface_mesh_parameterization::Error_code err = 
                ...
                if (err == CGAL::Surface_mesh_parameterization::OK) {
                    ...
                    for (auto v : vertices(cgal_mesh)) {
                        auto it = cgal_to_original_index.find(v);
                        if (it == cgal_to_original_index.end()) {
                            spdlog::warn("ARAP参数化(Discrete Authalic): 无法找到CGAL顶点的原始索引，跳过该顶点");
                            continue;
                        }
                        
                        size_t idx = it->second;
                        if (idx >= result.uv_coords.size()) {
                            spdlog::warn("ARAP参数化(Discrete Authalic): 顶点索引 {} 超出范围", idx);
                            continue;
                        }
                    ...
                    if (invalid_count > 0) {
                        spdlog::error("ARAP参数化(Discrete Authalic): 发现 {} 个无效UV坐标", invalid_count);
                        if (u_max > u_min && v_max > v_min) {
                            for (size_t i = 0; i < valid_uv.size(); ++i) {
                                if (!valid_uv[i]) {
                                    result.uv_coords[i].u = (u_min + u_max) * 0.5;
                    ...
                    spdlog::error("Discrete Authalic参数化也失败");
                    result.success = false;
                }
            } else {
                spdlog::error("无法找到网格边界");
